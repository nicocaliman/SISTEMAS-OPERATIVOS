...
passwd_entry_t* parse_passwd(struct options* options, int* nr_entries)
{
	...
	if ((passwd=fopen(options->input_file ? options->input_file :"/etc/passwd","r"))==NULL) {
	...
	while (fgets(line, MAX_PASSWD_LINE + 1, passwd) != NULL) {
		...
		while((token = strsep(&lineptr, ":"))!=NULL) {
			...
			case ENCRYPTED_PASS_IDX:
				cur_entry->optional_encrypted_passwd=strdup(token);	//clone_string(token);
				break;
			case UID_IDX:
			...
			case GID_IDX:
				...
				break;
			case USER_NAME_IDX:
				cur_entry->user_name= strdup(token); //clone_string(token);
				break;
			case USER_HOME_IDX:
				cur_entry->user_home= strdup(token); //clone_string(token);
				break;
			case USER_SHELL_IDX:
				/* remove new line from token */
				token[strlen(token)-1]='\0';
				cur_entry->user_shell= strdup(token);	//clone_string(token);
				break;
			default:
				break;
			}
...
/*
	el fichero /etc/passwd almacena en formato texto plano informacion esencial de los usuarios del sistema:

			-id numerico de usuario
			-id numerico de grupo
			...
	
	strdup() -> duplicar un string 
